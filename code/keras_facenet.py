...
def load_faces(directory):
    faces = list()
    # enumerate files
    for filename in [ f.name for f in scandir(directory)]:
        # path
...
def score_image(path, model_facenet, model, out_encoder, name='unknown'):
    ...
    # predict
    image_class = model.predict(imageX_norm)
    image_prob = model.predict_proba(imageX_norm)
...
def euclidean_class(path, model_facenet, out_encoder, emb, y, all_paths, name='unknown'):
    ...
    return faces, dists, predict_names, imageX_norm

def saveImage(new_name, new_emb, new_path, all_emb, all_path, all_y):
    all_emb.append(new_emb)
    all_path.append(new_path)
    all_y.append(new_name)
    np.savez_compressed(r'C:\Users\vczl048\keras_facenet\lfw\lfw-deepfunneled\embeddings.npz', all_emb, all_y, all_path)
